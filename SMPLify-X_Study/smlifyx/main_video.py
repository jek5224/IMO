...
from fit_single_frame_video import fit_single_frame
...
def main(**args):
    ...
    fit_obj = create_dataset(img_folder=img_folder, **args)
    ...
    for idx, data in enumerate(dataset_obj):
        ...
        for person_id in range(keypoints.shape[0]):
            ...
            final_params, _, _, _, loss = fit_single_frame(img, keypoints[[person_id]],
        ...
        betas_list.append(final_params[0].clone().detach().cpu())
        loss_list.append(loss)
        if num == 10:
            break

    # Ordinary Average
    # average_betas = betas_list[0]
    # for i in range(1, len(betas_list)):
    #     average_betas += betas_list[i]

    # average_betas /= len(betas_list)
    ...
    softmax_list = softmax(np.array(loss_list))
    betas_list = list(softmax_list * np.array(betas_list))
    ...
    for idx, data in enumerate(fit_obj):
        ...
        for person_id in range(keypoints.shape[0]):
            ...
            final_params, camera_opt_params, posed, unposed, _ = fit_single_frame(img, keypoints[[person_id]],
                             body_model=body_model,
                             camera=camera,
                             joint_weights=joint_weights,
                             dtype=dtype,
            ...
            # Does not initialize pose with previous pose
            #final_params[2] = None

            camera_positions_list.append(camera_opt_params)
            posed_list.append(posed)
            unposed_list.append(unposed)
    ...
    f = open('joints.txt', 'w')
    f.write(str(len(posed_list)) + '\n')
    for posed in posed_list:
        for joint in posed:
            f.write(str(np.round(-joint[0], 6)) + ' ' + str(np.round(-joint[1], 6)) + ' ' +str(np.round(-joint[2], 6)) +'\n')
        f.write('\n')
    f.write('\n')

    for unposed in unposed_list:
        for joint in unposed:
            f.write(str(np.round(-joint[0], 6)) + ' ' + str(np.round(-joint[1], 6)) + ' ' +str(np.round(-joint[2], 6)) +'\n')
        f.write('\n')
    f.write('\n')

    for pos in camera_positions_list:
        f.write(str(np.round(pos[0], 6)) + ' ' + str(np.round(pos[1], 6)) + ' ' +str(np.round(pos[2], 6)) +'\n')  
